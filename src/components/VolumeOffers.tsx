...
export function VolumeOffers({ items, quotes, weekNumber, onRefresh }: VolumeOffersProps) {
  ...
  // Use filterStandardSKUs to ensure normalization (e.g., 4x2lb -> 4×2 lb for strawberries)
  const standardItems = filterStandardSKUs(items);
  
  // Find quotes with offered_volume - handle both exact matches and normalization
  const itemsWithOffers = standardItems
    .map(item => {
      // Try exact match first
      let quote = quotes.find(q => q.item_id === item.id);
      
      // If no exact match, try finding by item name and pack_size (normalization fallback)
      if (!quote) {
        quote = quotes.find(q => {
          const qItem = q.item;
          if (!qItem) return false;
          return qItem.name?.toLowerCase() === item.name?.toLowerCase() &&
                 qItem.pack_size === item.pack_size &&
                 qItem.organic_flag === item.organic_flag;
        });
      }
    ...
    .filter(({ quote }) => {
      // Include quotes that have offered_volume > 0, even if supplier already responded
      // This ensures the component shows even if supplier hasn't responded yet
      return quote && quote.offered_volume !== null && quote.offered_volume !== undefined && quote.offered_volume > 0;
    });
  ...
  const handleSubmitAll = async () => {
    ...
    if (successCount > 0) {
      ...
      // Trigger navigation to volume acceptance tab for RF managers
      // This event is listened to by RFDashboard
      logger.debug('VolumeOffers: Dispatching navigate-to-volume-acceptance event', { weekNumber, successCount });
      window.dispatchEvent(new CustomEvent('navigate-to-volume-acceptance', {
        detail: { weekNumber, fromSupplierResponse: true }
      }));
    } else {
