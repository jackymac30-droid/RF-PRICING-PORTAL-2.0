    ...
    async function loadSuppliers() {
      try {
        ...
        if (suppliersData.length === 0) {
          ...
          logger.warn('No suppliers found:', { supabaseConfigured: !!supabaseUrl && !!supabaseKey });
        } else {
          setSupplierError('');
        }
      } catch (err: any) {
                  ...
                  <div>
                    ...
                    <div className="relative group">
                      <select
                        ...
                        className="w-full px-5 py-4 pr-12 text-base border-2 border-white/20 bg-white/10 backdrop-blur-sm rounded-xl focus:outline-none focus:ring-4 focus:ring-emerald-500/50 focus:border-emerald-400 transition-all text-white font-semibold appearance-none cursor-pointer hover:bg-white/15 hover:border-white/30 shadow-lg hover:shadow-xl z-10 relative"
                        required
                        style={{
                      ...
                      >
                        ...
                        <optgroup label="Suppliers" className="bg-slate-900">
                          {loadingSuppliers ? (
                            ...
                            <option disabled className="bg-slate-900 text-white">
                              {supplierError ? 'Check console for details' : 'No suppliers available'}
                            </option>
                          ) : (
                            suppliers.map(s => (
                    ...
                    {supplierError && (
                      <div className="mt-2 text-xs text-yellow-300 bg-yellow-500/20 border border-yellow-500/30 px-3 py-2 rounded-lg">
                        <strong>Note:</strong> {supplierError}
                      </div>
                    )}
                  </div>
