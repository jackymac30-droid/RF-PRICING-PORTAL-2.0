...
import { fetchWeeks } from '../utils/database';
...
export function VolumeAcceptance({ weekId }: VolumeAcceptanceProps) {
  ...
  // Wrap functions in useCallback to prevent infinite loops and ensure stable references
  const checkVolumeFinalized = useCallback(async () => {
    if (!weekId) return;
  ...
  const loadAllocations = useCallback(async () => {
    try {
      ...
      if (error) {
        logger.error(`Error loading allocations for week ${weekId}:`, error);
        throw error;
      }
      ...
      // REMOVED AUTO-ACCEPT: Don't auto-accept supplier responses - let RF manually accept them
      // This ensures responses show in "Supplier Responses" section so RF can review them
      // Auto-accepting immediately moves items to "finalized" and hides them from the "responded" section
      // RF should manually accept responses using the "Accept Response" button
      // const autoAcceptUpdates: Array<{ id: string; awarded_volume: number }> = [];
      // for (const q of data) {
      //   if (q.supplier_volume_response === 'accept' && 
      //       q.offered_volume && 
      //       q.offered_volume > 0 &&
      //       q.supplier_volume_accepted && 
      //       q.supplier_volume_accepted > 0 &&
      //       (!q.awarded_volume || q.awarded_volume === 0)) {
      //     autoAcceptUpdates.push({
      //       id: q.id,
      //       awarded_volume: q.supplier_volume_accepted
      //     });
      //   }
      // }
  ...
  }, [weekId, volumeFinalized, loadAllocations]);
  ...
  }, [weekId, checkVolumeFinalized, loadAllocations]);
  ...
  useEffect(() => {
    ...
    checkVolumeFinalized();
    loadWeek();
  }, [loadAllocations, checkVolumeFinalized, loadWeek]);
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          offered_volume: allocation.supplierVolumeAccepted, // Keep in sync
  ...
  // Include all responses (accept/update) where supplier has responded but RF hasn't finalized yet
  // Changed: Don't require awardedVolume === 0 - accept responses even if awardedVolume is set (they should still show)
  // But prioritize showing responses where RF hasn't accepted yet (awardedVolume === 0 or doesn't match accepted volume)
  const respondedAllocations = allocations.filter((a: VolumeAllocation) => {
    if (a.offeredVolume <= 0) return false;
    if (!a.supplierResponse || a.supplierResponse === 'decline') return false;
    // Show if supplier responded but RF hasn't finalized (awardedVolume is 0 or doesn't match accepted volume)
    // This ensures accepted responses show even if auto-accept happened
    return a.awardedVolume === 0 || (a.supplierVolumeAccepted > 0 && a.awardedVolume !== a.supplierVolumeAccepted);
  });
  // Separate declined allocations for display only (not included in financials)
  // Declined responses can have awarded_volume = 0 (withdrawn) or > 0 (kept as-is)
  const declinedAllocations = allocations.filter((a: VolumeAllocation) => 
    a.offeredVolume > 0 && 
    a.supplierResponse === 'decline'
  );
  
  // Finalized: has awarded_volume that matches supplier_volume_accepted (RF finalized the response)
  // Only show as finalized if:
  //   - Supplier declined (response is handled - RF decided to withdraw or keep) OR
  //   - Supplier responded (accept/update) AND awarded_volume matches supplier_volume_accepted (RF accepted supplier response)
  // NOTE: Do NOT include quotes where supplier hasn't responded yet - those are in "pendingAllocations"
  const finalizedAllocations = allocations.filter((a: VolumeAllocation) => {
    // Declined responses are always finalized (handled) regardless of awarded_volume
    // RF has decided: either withdraw (awarded_volume = 0) or keep (awarded_volume > 0)
    if (a.supplierResponse === 'decline') {
      return true; // Declined is finalized/handled
    }
    
    // For accept/update responses, only finalized if awarded matches accepted (RF accepted supplier response)
    if (a.supplierResponse && 
        a.supplierResponse !== 'decline' && 
        a.supplierVolumeAccepted > 0 &&
        a.awardedVolume > 0) {
      return a.awardedVolume === a.supplierVolumeAccepted; // RF accepted supplier response
    }
    
    // If supplier hasn't responded yet, this is NOT finalized - it's pending
    // (This should not happen if we're checking for "all responses handled", but included for completeness)
    return false;
  });
  ...
  // AUTO-CLOSE LOOP: If all responses are handled and loop isn't closed yet, automatically close it
  useEffect(() => {
    if (allResponsesHandled && !volumeFinalized && session?.role === 'rf' && !closingLoop && weekId) {
      // Small delay to ensure UI updates are complete
      const timer = setTimeout(async () => {
        logger.debug('All responses handled, auto-closing loop...');
        try {
          const result = await closeVolumeLoop(weekId, session.user_name);
          if (result.success) {
            showToast('Volume allocation loop closed automatically. Week is now locked.', 'success');
            setVolumeFinalized(true);
            await loadAllocations();
            await checkVolumeFinalized();
          }
        } catch (err) {
          logger.error('Error auto-closing loop:', err);
          // Don't show toast on auto-close failure - let user manually close if needed
        }
      }, 1500); // 1.5 second delay to let UI settle and auto-accepts complete

      return () => clearTimeout(timer);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [allResponsesHandled, volumeFinalized, session?.role, closingLoop, weekId]);

  // AUTO-CLOSE LOOP: If all responses are handled and loop isn't closed yet, automatically close it
  useEffect(() => {
    if (allResponsesHandled && !volumeFinalized && session?.role === 'rf' && !closingLoop) {
      // Small delay to ensure UI updates are complete
  ...
  }, [allResponsesHandled, volumeFinalized, session?.role, closingLoop]);
  ...
  // Calculate workflow state for WorkflowSteps component
  const hasAllocations = allocations.some(a => a.awardedVolume > 0);
  const allocationsSent = allocations.some(a => a.offeredVolume > 0);
  const suppliersResponded = allocations.some(a => a.supplierResponse);
    ...
    <div className="space-y-6">
      ...
      {week && (
        <WorkflowSteps
          ...
          suppliersResponded={suppliersResponded}
          loopClosed={volumeFinalized}
        />
      )}

      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-50 to-green-50 rounded-xl shadow-lg border-2 border-emerald-300 px-6 py-5">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-gradient-to-br from-emerald-500 to-green-600 rounded-lg shadow-lg">
            <CheckCircle className="w-6 h-6 text-white" strokeWidth={2.5} />
          </div>
          <div>
            <h2 className="text-2xl font-black text-gray-900">Volume Acceptance</h2>
            <p className="text-xs text-gray-600 font-semibold">Review and finalize supplier volume responses</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
