...
export default function AllocationResponse({ quotes, items, weekId, weekNumber, supplierId, onRefresh }: AllocationResponseProps) {
  ...
  logger.debug('=== ALLOCATION RESPONSE COMPONENT ===', {
    weekId,
    weekNumber,
    supplierId,
  ...
  const submitAllResponses = async () => {
    setSubmitting(true);
    logger.debug('=== SUBMIT ALLOCATION RESPONSES START ===', { weekId, supplierId, quotesCount: quotesWithAllocation.length });
...
if (errors.length > 0) {
      ...
      logger.debug('✓ All responses submitted successfully:', { successCount });
      ...
      logger.debug('AllocationResponse: Dispatching navigate-to-volume-acceptance event', { weekId, weekNumber, successCount });
      window.dispatchEvent(new CustomEvent('navigate-to-volume-acceptance', {
        detail: { weekId, weekNumber, fromSupplierResponse: true }
      }));
    } catch (error: any) {
      logger.error('=== ERROR SUBMITTING ALLOCATION RESPONSES ===', { error });
