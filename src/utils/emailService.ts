...
export async function sendPricingReminder(
  ...
  try {
    ...
    logger.debug('Sending pricing reminder', {
    ...
    if (resendApiKey) {
      ...
      logger.debug('Email would be sent (Resend API key not configured)', {
        to: recipientEmail,
        subject: emailTemplate.subject,
    ...
    if (auditError) {
      logger.error('Error logging reminder:', auditError);
    }

    return { success: true };
  } catch (error: any) {
    logger.error('Error sending pricing reminder:', error);
